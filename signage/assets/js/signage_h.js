...
// ============================================
//
// 本ファイルは横型サイネージ用のため、縦型とは以下の仕様差分あり。
// リファクタ・共通化時は注意すること。
//
// ■ 表示テキスト
// - 法人名 + 施設名を1行で結合して表示
...
//
// ■ 画像未設定時
// - 横型専用画像（noimage_h.png）を使用
//   （縦型は noimage.png）
//
// ■ 背景色の適用範囲
// - guide-back / guide-wrapper / table-label に適用
//   （縦型は html/body のみ）
//
// ■ レイアウト前提
// - 横幅が広いためフォントサイズ・レイアウト制約が異なる
//   （setFontSize のパラメータも別設計）
//
// ■ 画像エリア
// - 背景色（テーマカラー）を適用する仕様あり
//   （state.imageBackgroundColor を使用）
//
// =============================================
// ■ IE11対応方針
// =============================================
//
// 本ファイルはES6前提で実装している。
// IE11では以下の構文が非対応のため、そのままでは動作しない。
// ・テンプレートリテラル
// ・アロー関数
// ・スプレッド構文
// ・分割代入 など
...
(function () {
  ...
  // 画像がない場合に地図を表示するかどうか（現在は未使用・false固定）
  // ※有効化する場合はAPIのhttps対応が必要
